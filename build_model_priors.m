...
for j = 1:J
    ...
    % Use high-info mask from prepare_data
    gate = mask(:,j) & isfinite(r) & isfinite(dr) & (r > 0);
    
    if nnz(gate) < 8, continue; end
    ...
    % Nonlinearity direction: D*(») = qNH - NH projection
    % qNH stress: S_qNH = (1-3±)B + 2±·bracket(»)
...
%% Compute axis "needs" (data-driven, no knobs)

...
%% ========== DOMAIN & TOY TRUTH (for validation only) ==========
xmin = zeros(1,d); xmax = 2*pi*ones(1,d);
toUnit   = @(X) (X - xmin) ./ (xmax - xmin);
fromUnit = @(U) xmin + U .* (xmax - xmin);

% Positive integrand and its log10 (toy)
ftrue_nd   = @(X) exp(sum(sin(X),2) - d);
gtrue10_nd = @(X) log10(ftrue_nd(X));
I_true_analytic = (exp(-d)) * (2*pi*besseli(0,1))^d;

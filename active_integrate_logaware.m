...
%   uniform prior.
    ...
    while rounds < maxRounds && addedTotal < maxAdded
        ...
        % total relative half-width on I_rel
        sigma_rel_total_small = sqrt(Isd_model_small^2 + se_rqmc_small^2);
        relHalf_comb = 1.96 * sigma_rel_total_small;
        
        fprintf(['r=%2d  +%2d  C=%.2f gamma=%.2f qLoc=%d  ' ...
                 'logI_rel≈%.3g  modelSD_rel=%.3g  rqmcSE_rel=%.3g  half%%=%.2f%%  total=%d\n'], ...
    ...
    [logI_rel_bar, se_rqmc_final, Isd_model_final] = ...
        rqmc_integral_mean_and_SE(gpr, Nint_final, R_final, jacobian, d);
    
    % log I_rel from RQMC
    logI_mean = -N0 + logI_rel_bar;   % log Z = -N0 + log I_rel
    
    % Total *relative* uncertainty on I_rel (CHANGE #4 makes these relative)
    sigma_rel_total = sqrt(Isd_model_final^2 + se_rqmc_final^2);
    
    if isfinite(sigma_rel_total) && sigma_rel_total > 0
        % delta method: Var(log I) ≈ sigma_rel_total^2
        se_logI  = sigma_rel_total;
    ...
    out.I_mean       = I_mean;
    out.CI95         = CI95_lin;
    out.sigma_model  = Isd_model_final;
...
% INT_STATS_LOG10_FIXED - Log-space stable version
% 
% Returns logI_est = log(integral) instead of linear integral
% to prevent overflow when mu10 is large.

    ln10  = log(10);
    mu_e  = ln10*mu10;   % mu_e = ln(L_rel)
    sd_e  = ln10*sd10;
    
    % For uncertainty estimation (capped to prevent overflow)
    mu_e_safe = min(mu_e, 100);  % exp(100) ≈ 1e43
    sd_e_safe = min(sd_e, 10);
    Lmean = exp(mu_e_safe + 0.5*sd_e_safe.^2);
    Lvar  = exp(2*mu_e_safe + sd_e_safe.^2) .* (exp(sd_e_safe.^2) - 1);
    I_sd  = sqrt(sum(w.^2 .* Lvar));
    
    % Compute integral in log-space
    % I = Σ w_i × E[L_rel_i] = Σ w_i × exp(mu_e_i + 0.5×sd_e_i²)
    % log I = logsumexp(log(w_i) + mu_e_i + 0.5×sd_e_i²)
    log_w = log(max(w, realmin));
    log_integrand = log_w + mu_e_safe + 0.5*sd_e_safe.^2;
...
function [logI_bar, se_rqmc, I_sd_model_bar] = rqmc_integral_mean_and_SE(gpr, N, R, jacobian_fn, d)
% RQMC_INTEGRAL_MEAN_AND_SE_FIXED - Log-space stable version
%
% Returns logI_bar = log(mean integral) using log-space arithmetic

    logIvals = zeros(R,1); Isd2 = zeros(R,1);
    ...
    for r = 1:R
        ...
        w = jacobian_fn(U) / N;
        [mu, sd] = predict(gpr, U);
        [logI_r, I_sd_r] = int_stats_log10(mu, sd, w);
        logIvals(r) = logI_r;
        Isd2(r)  = I_sd_r.^2;
    end
    
    % Average in log-space using logsumexp
    ...
    % Now work in linear space only to get *relative* errors, which are bounded
    Ivals_linear = exp(logIvals);           % safe because int_stats_log10 clamps exponents
    Imean_linear = mean(Ivals_linear);
    
    % RQMC SE: relative (dimensionless)
    se_abs = std(Ivals_linear, 0) / sqrt(R);
    if Imean_linear > 0
        se_rqmc = se_abs / Imean_linear;    % relative SE on I_rel
    else
        se_rqmc = NaN;
    end
    
    % Model (GP) contribution: also relative
    I_sd_abs = sqrt(Isd2);                                  % per-scramble abs sigma
    I_sd_rel = I_sd_abs ./ max(Ivals_linear, realmin);      % convert to relative
    I_sd_model_bar = sqrt(mean(I_sd_rel.^2));               % relative model SD
